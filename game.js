        ...
        let swingRecorder = {
            ...
            cameraAngle: 'perspective' // Current camera view: 'perspective', 'top', 'front', 'left', 'right'
        };
        ...
        function initializeIMU() {
            ...
            updateStatus('✓ IMU Ready! Set ball position to start');
        }
        ...
        function handleSwingTimeout() {
            ...
            updateStatus('⏰ TIMEOUT! Set ball position again');
        }
        ...
        function registerBallHit() {
            ...
            updateStatus('🏌️ HIT! Ball is flying...');
        ...
        function calculateImpactVelocity() {
            ...
            // ====================================================================
            // Method: Finite difference - v = Δposition / Δtime
            ...
            // ====================================================================
            // Δposition = current - previous (meters)
            // velocity = Δposition / Δtime (m/s)
            // ====================================================================
            ...
            // ====================================================================
            // STEP 2: Transform phone coordinates → screen/world coordinates
            // ====================================================================
            ...
            if (settings.phoneOrientation === 'edge') {
                ...
                // Phone X-axis (along long edge) = swing direction → Screen Z (forward)
                // Phone Y-axis (screen normal) = left/right → Screen X
                // Phone Z-axis (top to bottom) = up/down → Screen Y
                const tempX = vx;
                const tempY = vy;
                const tempZ = vz;
                
                vx = tempY;   // Phone Y (screen normal) → Screen X (left/right)
                vy = -tempZ;  // Phone Z (top/bottom) → Screen Y (up/down), inverted
                vz = tempX;   // Phone X (long edge) → Screen Z (forward/back)
                
                addDebugMessage(`📱 Edge: PhoneX→ScreenZ, PhoneY→ScreenX, PhoneZ→ScreenY`);
                addDebugMessage(`📱 After rot: [${vx.toFixed(2)}, ${vy.toFixed(2)}, ${vz.toFixed(2)}] m/s`);
            }
            ...
            if (vy < 0) {
                ...
                addDebugMessage(`Loft applied: ${vy.toFixed(2)} → ${vyLofted.toFixed(2)} (${settings.loftAngle}°)`);
            } else {
            ...
            // ====================================================================
            // REAL PHYSICS: Ball speed / club speed ≈ 1.5x (due to club elasticity)
            // 
            ...
            // Real golf: club face spring effect ≈ 1.4-1.5x
            ...
            // ⚠️ SANITY CHECK: Warn if velocity seems unrealistic
            if (totalVel > 80) {
        ...
        function launchBall(initialVelocity) {
            ...
            // ====================================================================
            // Spin causes Magnus force → ball curves in flight
            ...
            if (ballRadius <= 0 || !isFinite(vx) || !isFinite(vy) || !isFinite(vz)) {
                ...
                // ================================================================
                // Physics: Sideways club motion → spin around Y-axis
                // Formula: ω = v / r (simplified - assumes rolling contact)
                // 
                // Spin direction (right-hand rule around Y-axis):
                // - Positive X velocity (right-to-left swing) → slice (right curve)
                // - Negative X velocity (left-to-right swing) → hook (left curve)
                // 
                ...
                // ================================================================
                // Physics: Upward club motion + loft → backspin
            ...
            if (ballFlight.velocity.y <= 0) {
                addDebugMessage(`⚠️ Y velocity ${ballFlight.velocity.y.toFixed(1)} ≤ 0! Ball won't go up!`);
            } else {
                addDebugMessage(`✓ Y velocity ${ballFlight.velocity.y.toFixed(1)} is UP`);
            }
            
            if (ballFlight.velocity.z <= 0) {
                addDebugMessage(`⚠️ Z velocity ${ballFlight.velocity.z.toFixed(1)} ≤ 0! Ball coming TOWARD you!`);
            } else {
                addDebugMessage(`✓ Z velocity ${ballFlight.velocity.z.toFixed(1)} is AWAY`);
            }
        }
        ...
        function updateBallPhysics(deltaTime) {
            ...
            // ====================================================================
            // F = ma  →  a = g = 9.81 m/s² (Earth standard)
            // v(t+dt) = v(t) + a*dt
            // ====================================================================
            ...
            // Formula: F_magnus = C_L * ω × v
            // Where:
            // - C_L: Lift coefficient (depends on ball properties, air density)
            // - ω: Angular velocity vector (spin rate, rad/s)
            ...
            if (settings.spinEffect > 0 && ballFlight.spin) {
                ...
                // ================================================================
                // Realistic C_L for golf ball ≈ 0.25 - 0.35
                ...
                // - Over 5 seconds: lateral velocity change ≈ 2.5 m/s
                // - Lateral displacement: ≈ 6 meters (realistic!)
                // ================================================================
                ...
                const spinDecay = 0.98; // Per frame (0.98^60 ≈ 0.3 after 1 second)
        ...
        // ================================================================
        function projectPoint(point, cameraAngle, scale, offsetX = 0, offsetY = 0) {
            let x, y;
        ...
        // ================================================================
        function drawAxes(centerX, centerY, length, cameraAngle) {
            const axes = [
        ...
        // Add stop replay button (long press or separate button could work too)
        canvas.addEventListener('click', function(e) {
            // If in replay mode and user clicks on canvas, stop replay
            if (swingRecorder.replayMode) {
        ...
        // ============================================
        // [1] IMU Permission & Data Collection - ✅ DONE
        // [2] IMU Data Processing - ✅ DONE - Store raw data
        // [3] Ball Position Setting - ✅ DONE - Visual feedback working
        // [4] Swing Detection - ✅ DONE - Madgwick filter sensor fusion implemented
        // [5] Hit Detection - ✅ DONE - Uses club tip position + ball radius + backswing check
        // [6] Velocity Calculation - ✅ DONE - Finite difference from position history
        ...
        // [9] Swing Visualization - ✅ DONE - Shows club tip trail and current position
        // [10] Results Display - ✅ DONE - Shows distance, max height, impact speed, timestamp
        // [11] Settings System - ✅ DONE - Full configuration panel with swing timeout
        // [12] Sound Effects - ✅ DONE - Hit, landing, and alarm sounds with volume control
        // [13] Data Persistence - ✅ DONE - Settings and last shot saved to localStorage
        // [14] Swing Timeout - ✅ DONE - Countdown timer with alarm to prevent IMU drift
        // [15] Club Tip Tracking - ✅ DONE - Madgwick filter + quaternion + kinematic model
